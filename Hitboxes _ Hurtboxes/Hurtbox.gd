...
func create_hit_effect(source):
	...
	if source == "CA":
		CA_effects.shuffle()
		effect = CA_effects[0].instantiate()
	elif source == "Enemy":
		Enemy_effects.shuffle()
		effect = Enemy_effects[0].instantiate()
	var main = get_tree().current_scene
	main.add_child(effect)
	var newPosition = Vector2.ZERO
	newPosition.x = global_position.x + randf_range(rand_x_min, rand_x_max)
	newPosition.y = global_position.y + randf_range(rand_y_min, rand_y_max)
